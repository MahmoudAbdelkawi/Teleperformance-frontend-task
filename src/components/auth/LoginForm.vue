...
async function validate(values: any, { setErrors }: any) {
    try {
        ...
        showSnackbar('Login successful', 'elevated', 'success');
    } catch (err: any) {
        if (err.response) {
...
function navigateToSignup() {
    router.push('/auth/register'); // Navigate to the signup page
}
</script>
...
<template>
    ...
    <div class="text-center mt-4">
        <v-btn text @click="navigateToSignup">{{ $t('Login.SignUp') }}</v-btn>
    </div>
</template>
